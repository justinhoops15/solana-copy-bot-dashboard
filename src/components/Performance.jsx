...
function LineTooltip({ active, payload }) {
  ...
  return (
    <div className="chart-tooltip">
      ...
      {d.symbol && <div className="chart-tooltip-row"><span className="muted">Token</span><span>{d.symbol}</span></div>}
    </div>
  );
}
...
// ── Trade row — display:contents on desktop, flex card on mobile ───────────
function TradeRow({ t }) {
  const win = t.pnl >= 0;
  ...
  return (
    <div className="perf-trade-row">
      {/* perf-tr-main / perf-tr-foot: display:contents on desktop, flex on mobile */}
      <div className="perf-tr-main">
        <span className="perf-trade-symbol">{t.symbol}</span>
        <span style={{ color: win ? "#4caf84" : "#e05c5c", fontWeight: 700,
          fontVariantNumeric: "tabular-nums", fontSize: 13 }}>
          {fmt(t.pnl)}
        </span>
        <span style={{ color: win ? "#4caf84" : "#e05c5c", fontSize: 12,
          fontVariantNumeric: "tabular-nums" }}>
          {fmtPct(t.pct)}
        </span>
      </div>
      <div className="perf-tr-foot">
...
export default function Performance() {
  ...
  useEffect(() => {
    setLoading(true);
    getPerformance(period).then(setData).catch(console.error).finally(() => setLoading(false));
  }, [period]);
  ...
  return (
    <div>
      <div className="section-head perf-section-head">
        <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
          <span className="section-title">Performance</span>
          <span className="section-count">{stats.total} trades</span>
        </div>
      ...
      <div className="perf-stats-grid">
        ...
        <StatCard label="Win Streak" value={stats.winStreak}
          sub={`Best: ${stats.maxStreak}`}
          color={stats.winStreak > 0 ? "#f5a623" : "#ffffff"} />
        <StatCard label="Best Wallet"
      ...
      <div className="perf-chart-card">
        ...
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={lineData} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
            ...
            <XAxis dataKey="date" tick={{ fill: "#444444", fontSize: 10 }}
              tickLine={false} axisLine={false} interval="preserveStartEnd" />
            <YAxis tick={{ fill: "#444444", fontSize: 10 }} tickLine={false} axisLine={false}
              tickFormatter={(v) => "$" + v.toFixed(2)} width={60} />
            <Tooltip content={<LineTooltip />} />
            <Line type="monotone" dataKey="cumPnl" stroke="#22d3ee" strokeWidth={2}
      ...
      <div className="perf-chart-card" style={{ padding: 0, overflow: "hidden" }}>
        <div className="perf-trade-head">
          ...
          <span>Wallet</span><span>Dur</span><span>Date</span>
        </div>
        {trades.length === 0 ? (
          ...
          <>
            {(showAll ? trades : trades.slice(0, 20)).map((t) => <TradeRow key={t.id} t={t} />)}
            {trades.length > 20 && (
